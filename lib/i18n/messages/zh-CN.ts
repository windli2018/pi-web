...
export const zhCNLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "services.close": "关闭",
    "services.hint": "虚拟主机链接的端口自动取自当前页面 URL。泛域名解析配置见上方说明。",
    ...
    "services.configTitle": "泛域名配置说明",
    "services.expose": "暴露到公网（DNS / 隧道）",
    "services.exposeLocal": "本机：*.pi.localhost 零配置（RFC 6761，浏览器自动映射 127.0.0.1）。",
    "services.exposeLan": "局域网：dnsmasq 加 address=/pi.lan/<服务器IP>，并设置 PI_WEB_SVC_HOST_SUFFIX、PI_WEB_ALLOWED_HOSTS。",
    "services.exposePublic": "公网域名：DNS 加通配 A 记录 *.pi.example.com → 服务器IP，域名加入 PI_WEB_ALLOWED_HOSTS。",
    "services.exposeTunnel": "隧道只暴露单个服务端口（或其虚拟主机子域名）——绝不是 pi-web 本体。启动 pi-web 时用 PI_WEB_TUNNELS（JSON）/ PI_WEB_TUNNEL_CMD 指向服务端口，或在下方端口卡片里就地开启。",
    ...
    "services.auth": "暴露前的授权管理（推荐）",
    "services.authBody": "1. Authelia —— 自托管 SSO + 2FA 门户，配合 Caddy/nginx 的 forward_auth（推荐）\n2. Authentik —— 开源身份平台（OIDC/SAML/LDAP）\n3. oauth2-proxy —— 轻量反向代理认证，对接 Google/GitHub/OIDC\n4. Keycloak —— 企业级 IAM（较重）\n5. Cloudflare Access —— 托管式零信任网关",
    "services.authHint": "在暴露任何服务前，请先为 pi-web（或隧道）配置 HTTPS 与认证。",
    "services.configLocal": "本机：*.pi.localhost 无需 DNS，浏览器自动映射到 127.0.0.1（RFC 6761）。",
    "services.configLan": "局域网：在 dnsmasq 加一行 address=/pi.lan/<服务器IP>，并设置 PI_WEB_SVC_HOST_SUFFIX=.pi.lan、PI_WEB_ALLOWED_HOSTS=pi.lan。",
    "services.configPublic": "公网域名：在 DNS 加通配 A 记录 *.pi.example.com → 服务器IP，并把域名加入 PI_WEB_ALLOWED_HOSTS。",
    "services.configCurrent": "当前页面通过 {host} 访问，虚拟主机地址形如 http://<端口>{host}{pagePort}{basePath}。",
    "sidebar.hide": "隐藏侧边栏",
    "sidebar.show": "显示侧边栏",
    "theme.light": "切换到浅色模式",
    "theme.dark": "切换到深色模式",
