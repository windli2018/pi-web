...
export const enLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "services.exposeSection": "Public access (auth gateway)",
    "services.exposeMode": "Mode",
    "services.exposeModeTunnel": "tunnel (public URL)",
    "services.exposeModeProxy": "proxy (local vhost, no tunnel)",
    "services.exposeModeBoth": "both (local + tunnel)",
    "services.exposeSite": "Site (auth realm)",
    "services.exposeSiteDefault": "default",
    "services.allowToolDownload": "Allow online tool downloads",
    "services.allowToolDownloadHint": "When disabled, only tools already installed on this machine are used; missing tools are not downloaded automatically.",
    "services.toolsMissing": "missing",
    "services.exposeProtocol": "Protocol",
    "services.selectProvider": "Select provider…",
    "services.exposeProvider": "Auth provider",
    "services.exposeProviderNone": "none (no auth)",
    "services.exposeLogs": "Logs",
    "services.exposeLogsHide": "Hide logs",
    "services.exposeStop": "Stop",
    "services.exposeAuth": "Require sign-in",
    "services.exposeTunnelTool": "Tunnel",
    "services.tunnelParams": "Tunnel params",
    "services.recommended": "Recommended",
    "services.authParams": "Auth params",
    "services.exposeGo": "Expose now",
    "services.exposeResult": "Live at",
    "services.openLogin": "Open login page",
    "services.authUsers": "Auth & users",
    "services.userList": "Users",
    "services.addUser": "Add user",
    "services.userName": "Username",
    "services.userPassword": "Password",
    "services.userDisplayName": "Display name",
    "services.userRemove": "Remove",
    "services.userSetPassword": "Set password",
    "services.newPassword": "New password",
    "services.update": "Update",
    "services.totpEnroll": "Enable 2FA",
    "services.totpDisable": "Disable 2FA",
    "services.totpEnrolled": "2FA on",
    "services.totpOff": "2FA off",
    "services.totpSecret": "Scan with your authenticator app",
    "services.totpManual": "Or enter the secret manually",
    "services.totpActivateHint": "Enter the 6-digit code from your authenticator to activate — 2FA is NOT enforced until verified.",
    "services.totpCode": "6-digit code",
    "services.totpActivate": "Activate",
    "services.activating": "Activating…",
    "services.totpActivateFailed": "Invalid code",
    "services.totpApps": "Authenticator apps (CN-store friendly)",
    "services.totpAppMs": "Microsoft Authenticator",
    "services.totpAppCnStore": "(installable from CN stores: Huawei/Xiaomi/AppBao)",
    "services.totpAppTencent": "Tencent Authenticator (search app stores)",
    "services.totpAppGoogle": "Google Authenticator",
    "services.docs": "Docs",
    "services.totpApkMirror": "CN mirror APK download (SHA-256 verified)",
    "services.otpauthOpen": "otpauth://… (open in authenticator)",
    "services.officialSite": "Official site",
    "services.appStore": "App Store",
    "services.noUsers": "No users yet — add the first one below.",
    "services.adding": "Adding…",
    "services.updating": "Updating…",
    "services.exposing": "Exposing…",
    "services.exposeDone": "Done",
    "services.exposeFailed": "Expose failed",
    "services.providerChanged": "Switched provider",
    "services.title": "Service ports",
    "services.tabPorts": "Ports",
    "services.tabUsers": "Users",
    "services.auto": "Auto-refresh every 10s",
    "services.loading": "Scanning...",
    "services.none": "No service ports found. Servers started by pi sessions appear here automatically.",
    "services.open": "Open",
    "services.refresh": "Refresh",
    "services.ngrokAuthtoken": "ngrok authtoken",
    "services.ngrokAuthtokenPlaceholder": "paste NGROK_AUTHTOKEN (required)",
    "services.ngrokAuthtokenGet": "get token",
    "services.error": "Failed to scan service ports",
    "services.close": "Close",
    "services.direct": "Direct (bypass pi-web)",
    ...
    "services.infoAccess": "How other devices reach these services (DNS / wildcard domain)",
    "services.infoAuth": "Authentication guide",
    "services.exposeLocal": "Same machine: *.pi.localhost zero-config (RFC 6761).",
    "services.exposeLan": "LAN: add \"address=/pi.lan/<server-ip>\" to dnsmasq, then set PI_WEB_SVC_HOST_SUFFIX=.pi.lan and PI_WEB_ALLOWED_HOSTS=pi.lan.",
    "services.exposePublic": "Public domain: wildcard A record *.pi.example.com → server IP, add the domain to PI_WEB_ALLOWED_HOSTS.",
    "services.exposeTunnel": "Tunnels expose ONE service port (or its virtual-host subdomain) — never pi-web itself. Pick a tunnel type + auth provider in the port card's “Public access” section to expose in place. Never point a tunnel at pi-web's own port: the host gate would 403 it unless whitelisted, and whitelisting it would expose pi-web unauthenticated.",
    "services.tunnelStatus": "Tunnel",
    "services.tunnelRunning": "running",
    "services.tunnelStopped": "exited",
    "services.tunnelStart": "Start",
    "services.tunnelStop": "Stop",
    "services.portTunnel": "Tunnel this port (exposes only it)",
    "services.activePortTunnels": "Open port tunnels",
    "services.activeExposes": "Active exposes",
    "services.details": "Details",
    "services.needsAuth": "needs token",
    "services.needsAuthHint": "Free signup at ngrok.com, then run: ngrok config add-authtoken <token>",
    "services.portTunnelHint": "Only this port is exposed. Never tunnel pi-web itself without authentication.",
    "services.authHint": "Add HTTPS and authentication in front of pi-web (or the tunnel) before exposing anything.",
    "services.authBuiltinTitle": "Built-in (selectable in the port card's “Expose now”)",
    "services.authBuiltinPortal": "portal — built-in auth portal: user management + TOTP 2FA (manage users in the Users tab)",
    "services.authBuiltinBasic": "basic — HTTP Basic auth: username + password",
    "services.authBuiltinAuthelia": "authelia — self-hosted Authelia via forward_auth (unavailable on Windows)",
    "services.authExtendTitle": "Extendable — configure in front of pi-web / the tunnel (nginx or Caddy)",
    "services.authExtendList": "Authentik — open-source identity platform (OIDC/SAML/LDAP)\noauth2-proxy — lightweight reverse-proxy auth via Google/GitHub/OIDC\nKeycloak — enterprise IAM (heavier)\nCloudflare Access — managed zero-trust gateway",
    "services.configCurrent": "This page is served from {host} — {host} virtual hosts are enabled automatically, e.g. http://<port>{host}{pagePort}{basePath}.",
    ...
    "chat.queueCollapse": "Collapse queued messages",
    "chat.queueExpand": "Expand queued messages",
    "chat.recall": "Recall to input",
    "chat.recallTitle": "Remove all queued messages and put them back into the input box for editing",
    "chat.queueExport": "Export queue",
    "chat.queueImport": "Import queue",
    "chat.queueRemove": "Remove this message",
    ...
    "chat.queueRecoveryTitle": "Pending queued messages · {count}",
    "chat.queueRecoveryDesc": "These messages were in the queue when the server restarted and were never processed. Nothing is delivered automatically — pick what to do with each one.",
    "chat.queueRecoveryDescShort": "Messages left in the queue after a restart.",
    "chat.queueImportTitle": "Imported queued messages · {count}",
    "chat.queueImportDesc": "These messages came from an imported file. They won't run automatically — decide what to do with each.",
    ...
    "chat.queueRecoverySelectNone": "Select none",
    "chat.queueRecoveryRequeue": "Re-queue selected",
    "chat.queueRecoveryRequeueContinue": "Re-queue & continue",
    "chat.queueRecoveryRequeueContinueTitle": "Re-queue and let the agent process them now",
    "chat.queueRecoveryDiscard": "Discard selected",
    "chat.queueRecoveryExportMd": "Export .md",
    "chat.queueRecoveryExportJson": "Export .json",
    "chat.queueRecoveryDismiss": "Later",
    "chat.queueRecoveryDismissTitle": "Keep them pending and ask again next time",
    "chat.recalledEditing": "Editing a message pulled from the queue — sending puts it back at position {pos}",
    "chat.recalledRequeue": "Re-queue",
    "chat.recalledCancel": "Cancel",
    ...
    "chat.minimizeQueue": "Collapse queue panel",
    "chat.minimizeInput": "Collapse input panel",
    "chat.restoreBottom": "Expand all",
    "chat.bottomInputCollapsed": "Input collapsed, tap the bar on the right to expand",
    "chat.swipeExpand": "Keep swiping up to expand",
    "chat.swipeCollapse": "Keep swiping down to collapse",
    "chat.shell": "Shell",
    ...
    "chat.compactQueued": "Queued",
    "chat.cancelCompactQueue": "Click to cancel queued compaction",
    "chat.changeModel": "Change model",
    "chat.modelSwitchStreaming": "Change model (applies next turn)",
    "chat.modelSwitchPendingBadge": "next turn",
    ...
    "export.queueTab": "Queue",
    "export.dateFrom": "From",
    "export.dateTo": "To",
    "export.selectAll": "Select all",
    "export.selectInvert": "Invert",
    "export.loading": "Loading…",
    "export.empty": "Nothing to export",
    "export.perPage": "Per page",
    "export.selected": "{count} selected",
    "export.button": "Export {count}",
    "export.busy": "Exporting…",
    "export.done": "Exported {count} items",
    "export.noneSelected": "Select items to export first",
    "export.close": "Close",
    "export.revealDir": "Reveal folder",
