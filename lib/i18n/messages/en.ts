...
export const enLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "services.auto": "Auto-refresh every 3s",
    ...
    "services.hint": "The port in virtual-host URLs comes from this page's URL automatically. See the wildcard-domain setup above.",
    ...
    "services.configTitle": "Wildcard domain setup",
    "services.expose": "Expose to public (DNS / tunnel)",
    "services.exposeLocal": "Same machine: *.pi.localhost zero-config (RFC 6761).",
    "services.exposeLan": "LAN: add \"address=/pi.lan/<server-ip>\" to dnsmasq, then set PI_WEB_SVC_HOST_SUFFIX and PI_WEB_ALLOWED_HOSTS.",
    "services.exposePublic": "Public domain: wildcard A record *.pi.example.com → server IP, add the domain to PI_WEB_ALLOWED_HOSTS.",
    "services.exposeTunnel": "Tunnels expose ONE service port (or its virtual-host subdomain) — never pi-web itself. Start pi-web with PI_WEB_TUNNELS (JSON) / PI_WEB_TUNNEL_CMD pointing at a service port, or open a tunnel right from the port card below.",
    "services.exposeTunnelExamples": "PI_WEB_TUNNEL_CMD=\"lt --port 8901\"\nPI_WEB_TUNNEL_CMD=\"/home/wind/pi-tunnels/cloudflared tunnel --url http://127.0.0.1:8901\"\nPI_WEB_TUNNEL_CMD=\"ssh -R 80:127.0.0.1:8901 serveo.net\"\nPI_WEB_TUNNELS='[{\"name\":\"cfd\",\"cmd\":\"cloudflared tunnel --url http://127.0.0.1:8901\",\"auto\":true}]'",
    ...
    "services.auth": "Access control before exposing (recommended)",
    "services.authBody": "1. Authelia — self-hosted SSO + 2FA portal; pair with Caddy/nginx forward_auth (recommended)\n2. Authentik — open-source identity platform (OIDC/SAML/LDAP)\n3. oauth2-proxy — lightweight reverse-proxy auth via Google/GitHub/OIDC\n4. Keycloak — enterprise IAM (heavier)\n5. Cloudflare Access — managed zero-trust gateway",
    "services.authHint": "Add HTTPS and authentication in front of pi-web (or the tunnel) before exposing anything.",
    "services.configLocal": "Same machine: *.pi.localhost needs no DNS — browsers map it to 127.0.0.1 (RFC 6761).",
    "services.configLan": "LAN: add \"address=/pi.lan/<server-ip>\" to dnsmasq, then set PI_WEB_SVC_HOST_SUFFIX=.pi.lan and PI_WEB_ALLOWED_HOSTS=pi.lan.",
    "services.configPublic": "Public domain: create a wildcard A record like *.pi.example.com → server IP, and add the domain to PI_WEB_ALLOWED_HOSTS.",
    "services.configCurrent": "This page is served from {host} — virtual-host URLs look like http://<port>{host}{pagePort}{basePath}.",
