...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const handleFollowUp = useCallback(async (message: string, images?: Array<{ data: string; mimeType: string }>) => {
    const sid = sessionIdRef.current;
    if (!sid) return;
    const piImages = images?.map((img) => ({ type: "image" as const, data: img.data, mimeType: img.mimeType }));
    try {
  ...
  const resolveRecovery = useCallback(async (
    ...
    try {
      ...
      void (async () => {
        try {
          const stateRes = await fetch(apiUrl(`/api/agent/${encodeURIComponent(sid)}`));
  ...
  const scrollToBottom = useCallback((behavior: ScrollBehavior = "smooth") => {
    ...
    // The end sentinel sits BELOW the agent-running spacer (96px).
    // scrollIntoView on the sentinel would put that blank spacer in the
    // viewport — hence the blank screen while follow-streaming during a run.
    // Back off by the spacer + viewport height so the LAST MESSAGE lands
  ...
  useEffect(() => {
    if (session) {
      ...
      loadSession(session.id, true, true).then((agentState) => {
        ...
        void fetch(apiUrl(`/api/sessions/${encodeURIComponent(session.id)}/queue-recovery`))
  ...
  // viewport (40px keep-out below), so a visible last message does not cause
